defmodule Butterbeer.Location do
  ...
  def get_place_id!(%{name: name, google_maps_link: google_maps_link, area_level_two_id: area_level_two, url: url, neighborhood: neighborhood} = params) do
    query = from a in "places", select: a.id, where: a.area_level_two_id == ^area_level_two and ilike(a.google_maps_link, ^google_maps_link)
    output = Repo.all(query)
    if output == [] do
